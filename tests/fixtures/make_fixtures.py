...
class Builder:
    ...
    def clean_source(self, spectype: str, *, program="dark", healpix=1234, z=None, zwarn=0,
        ...
        self.expect(nrow, tid, spectype, has_spectrum=with_spectrum, has_cutout=with_cutout)
    ...
    def build(self) -> dict:
        ...
        # a flux consistent with zero in P3 on clean[8]: LOWERR >= FLUX
        m8 = next(m for m in self.main if m["DETUID"] == clean[8]["detuid"])
        m8["ML_FLUX_LOWERR_P3"] = m8["ML_FLUX_P3"] * 1.2
